...
const float ANIMATION_SPEED = 1.4;  // Velocidade de animação para efeito colorido
...
bool isOuterEdge(vec2 uv, float blurSize) {
    vec4 originalColor = texture2D(u_Tex0, uv);
    vec4 blurredColor = blurTexture(uv, blurSize);
...
void main() {
    ...
    if (isOuterEdge(v_TexCoord, pulsatingSize)) {
