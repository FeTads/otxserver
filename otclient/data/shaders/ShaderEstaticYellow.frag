...
const float ANIMATION_SPEED = 1.4;
...
void main() {
    ...
    if (isOuterEdge(v_TexCoord)) {
        float continuousPhase = mod(u_Time * ANIMATION_SPEED, 1.0);
        float verticalPhase = mod(v_TexCoord.y + continuousPhase, 1.0);
        
        float phase = 10.0 * (v_TexCoord.x + verticalPhase);
        float y = sin(phase) * 0.5 + 0.5; // Intensity of yellow
        vec4 movingColor = vec4(y, y, 0.0, 1.0); // Yellow color

        // Base color slightly transparent
        vec4 baseColor = vec4(0.1, 0.1, 0.0, 0.2); // Darker yellow tone
        gl_FragColor = baseColor + movingColor * (1.0 - baseColor.a); // Blend the moving color over the base color
    } else {
        ...
        if (texcolor3.a > ALPHA_TOLERANCE && gl_FragColor.a < ALPHA_TOLERANCE) {
            gl_FragColor = vec4(1.0, 1.0, 0.0, 1.0); // Full yellow with alpha
            gl_FragColor.a = 0.7;
        }
