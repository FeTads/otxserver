...
function UIMessageBox.display(title, message, buttons, onEnterCallback, onEscapeCallback)
  ...
  local highlightData = getNewHighlightedText(message, "white", "#ffffff")
  if #highlightData > 2 then
    messageLabel:setColoredText(highlightData)
  end

  local highlightData = getNewHighlightedText(title, "white", "#ffffff")
  if #highlightData > 2 then
    messageBox:setColoredText(highlightData)
  end
