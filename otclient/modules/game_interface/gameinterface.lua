...
function init()
  ...
  -- closele = gameRootPanel:getChildById('closele')
  -- closele:setVisible(false)
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if creatureThing then
    ...
    menu:addSeparator()
      ...
      menu:addOption(tr('tecnicas'), function() modules.game_console.sendMessage("!tecnicas") end)
    end
	
    if creatureThing:isLocalPlayer() then
      menu:addOption(tr('Set Outfit'), function() g_game.requestOutfit() end)
	  
		menu:addOption(tr("HealthBar"), function ()
			g_game.talk("/checkbar")
		end)
		
		if creatureThing:isLocalPlayer() then
		  menu:addOption(tr('BackAura'), function() g_game.talk("/backaura") end)end  
		  
		if creatureThing:isLocalPlayer() then
		  menu:addOption(tr('Shaders'), function() g_game.talk("/shaders") end)end  
		  
      if activeloot == false then
        menu:addOption(tr('Autoloot OFF'), function() g_game.talk("!autoloot off") activeloot = true end)
    else
        menu:addOption(tr('Autoloot ON'), function() g_game.talk("!autoloot on") activeloot = false end)
    end
      ...
      if creatureThing:getPosition().z == localPosition.z then
        if g_game.getAttackingCreature() ~= creatureThing then
          menu:addOption(tr('Attack'), function() attackLuIzfkpF(creatureThing) end, shortcut)
        else
  ...
  if g_app.isMobile() then
    ...
    elseif action == "attack" then
      if attackCreature and attackCreature ~= player then
        resetLeftActions()
        attackLuIzfkpF(attackCreature)
        return true
      elseif creatureThing and creatureThing ~= player and creatureThing:getPosition().z == autoWalkPos.z then
        resetLeftActions()
        attackLuIzfkpF(creatureThing)
        return true
      end
      return true
